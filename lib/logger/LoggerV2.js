...
const log = async (method, args, file, logDateAndHours) => {
    ...
    if (typeof args == "object") {
        if (args.date === undefined || args.date == true)
            switch (method) {
                case "log":
                    args.content = `${style["%FW"]}[${date}/${style["%FG"]}LOG${style["%FW"]}]${style["%FG"]} ${args.content} ${style["%UR"]}`;
                    break;
                case "info":
                    args.content = `${style["%FW"]}[${date}/${style["%FC"]}INFO${style["%FW"]}]${style["%FC"]} ${args.content} ${style["%UR"]}`;
                    break;
                case "warn":
                    args.content = `${style["%FW"]}[${date}/${style["%FY"]}WARN${style["%FW"]}]${style["%FY"]} ${args.content} ${style["%UR"]}`;
                    break;
                case "error":
                    args.content = `${style["%FW"]}[${date}/${style["%FR"]}ERROR${style["%FW"]}]${style["%FR"]} ${args.content} ${style["%UR"]}`;
                    break;
            }
        ...
        switch (method) {
            case "log":
                args = `${style["%FW"]}[${date}/${style["%FG"]}LOG${style["%FW"]}]${style["%FG"]} ${args} ${style["%UR"]}`;
                break;
            case "info":
                args = `${style["%FW"]}[${date}/${style["%FC"]}INFO${style["%FW"]}]${style["%FC"]} ${args} ${style["%UR"]}`;
                break;
            case "warn":
                args = `${style["%FW"]}[${date}/${style["%FY"]}WARN${style["%FW"]}]${style["%FY"]} ${args} ${style["%UR"]}`;
                break;
            case "error":
                args = `${style["%FW"]}[${date}/${style["%FR"]}ERROR${style["%FW"]}]${style["%FR"]} ${args} ${style["%UR"]}`;
                break;
        }
        console.log(args);
        writeLog(args, file);
